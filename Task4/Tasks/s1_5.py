...
def optuna_model_s1_5(trial, input_shape, classes, activation, regularizer, initializer):
...
def objective(trial, train_dataset, validation_dataset, epochs, input_shape, classes, activation, regularizer, initializer):
    model = optuna_model_s1_5(trial, input_shape, classes, activation, regularizer, initializer)
    model.compile(loss='categorical_crossentropy', optimizer=tf.keras.optimizers.SGD(learning_rate=0.05), metrics=['accuracy'])
    # checkpoint = ModelCheckpoint('best_model_s1_5.keras', save_best_only=True, monitor='val_accuracy', mode='max')
    history = model.fit(
        train_dataset,
        epochs=epochs,
...
try:
    optuna.delete_study(storage='sqlite:///optuna.db', study_name='s1_5')
except:
    pass
study = optuna.create_study(storage='sqlite:///optuna.db', study_name= "s1_5", direction='maximize')
