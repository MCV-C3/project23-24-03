...
def lr_schedule(epoch, lr):
    if epoch < 15:
        ...
        return lr * tf.math.exp(-0.1 * (epoch - 14))
...
def objective(trial, input_shape, train_dataset, validation_dataset, classes, epochs, optimizer, lr, activation, regularizer, initializer):
    ...
    checkpoint = ModelCheckpoint('best_model_s4_1b.keras', save_best_only=True, monitor='val_accuracy', mode='max')
...
try:
    optuna.delete_study(storage='sqlite:///optuna.db', study_name='s4_1b')
except:
    pass
study = optuna.create_study(storage='sqlite:///optuna.db', study_name= "s4_1b", direction='maximize')
