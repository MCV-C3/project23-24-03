...
try:
    optuna.delete_study(storage='sqlite:///optuna.db', study_name='s3_2')
except:
    pass
study = optuna.create_study(storage='sqlite:///optuna.db', study_name= "s3_2", direction='maximize')
study.optimize(lambda trial: objective(trial, (IMG_HEIGHT, IMG_WIDTH, 3), 10, activation, regularizer, initializer), n_trials=60, n_jobs=1)
