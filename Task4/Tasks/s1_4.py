...
def optuna_model_s1_4(trial, input_shape, classes, activation, regularizer, initializer):
    ...
    for i in range(n_blocks):
        ...
        block_type = trial.suggest_categorical(f'block_{i}_type', ['inception', 'separable'])
        if block_type == 'separable':
            filters = trial.suggest_categorical(f'block_separable_{i}_filters', [16, 32, 48, 64, 96, 128, 256])
            x, residual = separable_res_block(x, residual, filters, stride, activation, regularizer, initializer)
        
        if block_type == 'inception':
            inception_type = trial.suggest_categorical(f'block_inception_{i}', ['i1', 'i2', 'i3'])
            if inception_type == 'i1':
                x, residual = seprable_inception_res_block1(x, residual, activation, regularizer, initializer)
            if inception_type == 'i2':
                x, residual = seprable_inception_res_block2(x, residual, activation, regularizer, initializer)
            if inception_type == 'i3':
                x, residual = seprable_inception_res_block3(x, residual, activation, regularizer, initializer)
...
def objective(trial, train_dataset, validation_dataset, epochs, input_shape, classes, activation, regularizer, initializer):
    model = optuna_model_s1_4(trial, input_shape, classes, activation, regularizer, initializer)
    model.compile(loss='categorical_crossentropy', optimizer=tf.keras.optimizers.SGD(learning_rate=0.05), metrics=['accuracy'])
    # checkpoint = ModelCheckpoint('best_model_s1_4.keras', save_best_only=True, monitor='val_accuracy', mode='max')
    history = model.fit(
        train_dataset,
        epochs=epochs,
...
try:
    optuna.delete_study(storage='sqlite:///optuna.db', study_name='s1_4')
except:
    pass
study = optuna.create_study(storage='sqlite:///optuna.db', study_name= "s1_4", direction='maximize')
